...
### Armo base final
jugadores_wiki <- readr::read_rds("data/lista_convocados_wikipedia.rds") |> 
  filter(mundial == anio) |> 
...
### Escribo seleccion
if(file.exists(glue::glue("data/seleccion_{anio}.rds"))){
  
  file.remove(glue::glue("data/seleccion_{anio}.rds"))
  
  write_rds(seleccion_def,
            glue::glue("data/seleccion_{anio}.rds"))
} else {
